...
(function ($) {
    ...
    var methods = {
        init: function (options, swfUploadOptions) {
           //  $("#uploadBackImg-queue").hide();//范永超修改 自定义皮肤去掉进度条
            return this.each(function () {
                ...
                var settings = $.extend({
                    ...
                    fileTypeDesc: '支持类型',        // The description for file types in the browse dialog
                ...
                if (flashInstalled) {
                    ...
                    // Wrap the instance
                    var $wrapper = $('<div id="'+settings.id+'" class="uploadify"></div>', {                        
                        'css': {
                    ...
                    // Create the button
                    var $button = $('<button id="'+settings.id+'-button" class="tn-button tn-corner-all tn-button-text-only tn-button-large tn-button-primary '+ settings.buttonClass+'" ></button>');
                    ...
                    $button.html('<span class="tn-button-text">' + settings.buttonText + '</span>')
					.css({
					    //'height': settings.height + 'px',
                    ...
                    if (!settings.queueID) {
                        var $queue = $('<div id="'+ settings.id +'-queue" class="uploadify-queue"></div>');
                        $wrapper.after($queue);
    ...
    var handlers = {
        ...
        onDialogOpen: function () {
            ...
            this.queueData.errorMsg = '文件没有添加到上传列表：';
        ...
        onSelect: function (file) {
            ...
            for (var n in this.queueData.files) {
                ...
                if (queuedFile.uploaded != true && queuedFile.name == file.name) {
                    var replaceQueueItem = confirm('你希望重新上传文件  "' + file.name + '" 吗?');
                    if (!replaceQueueItem) {
            ...
            if ($.inArray('onSelect', settings.overrideEvents) < 0) {
                ...
                $('#' + settings.queueID).append('<div id="' + file.id + '" class="uploadify-queue-item tn-widget-content tn-border-gray">\
					<div class="cancel">\
                ...
                switch (errorCode) {
                    case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
                        if (settings.queueSizeLimit > errorMsg) {
                            ...
                            this.queueData.errorMsg += '\n只能同时添加' + settings.queueSizeLimit + '个文件进行上传.';
                        }
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                     var content ;
                      if(settings.fileSizeLimit>1024)
                      {
                        content =  settings.fileSizeLimit/1024 +"MB";
                       }
                       else
                       {
                          content = settings.fileSizeLimit+"kB";
                       }
                        this.queueData.errorMsg += '\n文件 "' + file.name + '" 超出了容量 ' + content + '的大小限制.';
                        break;
                    case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
                        this.queueData.errorMsg += '\n文件 "' + file.name + '" 的大小不能为0KB！';
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
