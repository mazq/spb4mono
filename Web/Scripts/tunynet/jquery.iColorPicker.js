(function($) {
    function iColorShow(id, id2) {
        var eICP = $("#" + id2).offset();
        $("#iColorPicker").css({
            'top': eICP.top + ($("#" + id).outerHeight()) + "px",
            'left': (eICP.left) + "px",
            'position': 'absolute'
        }).fadeIn("fast");
        $("#iColorPickerBg").css({
    ...
    $.fn.iColorPicker = function() {
        ...
        .after('<a href="javascript:void(0)" id="icp_' + this.id + '"><span class="tn-icon tn-icon-color tn-icon-inline"></span></a>')
        .next('a:first').click(function() {
            iColorShow(inputid, this.id);
            $(document).one('click', function() { $("#iColorPicker").fadeOut() });
            return false;
        });
        });
    };
    $(function() {
        $("input.iColorPicker").iColorPicker();
    });
})(jQuery);